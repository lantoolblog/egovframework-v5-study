 ...
 *
 * <p>AiServices 인터페이스의 반환 타입을 POJO(record)로 선언하면 <br>
 * LangChain4j가 자동으로 JSON 스키마를 생성하고 응답을 역직렬화. <br>
 *
 * <pre>{@code
 * Spring AI 비교: <br>
...
public class StructuredOutputServiceImpl implements StructuredOutputService {
  ...
  /** AiServices내부 인터페이스: 반환 타입이 POJO이면 LangChain4j가 자동으로 JSON 스키마 생성 */
  private interface BookInfoExtractor {
    BookInfo extractBookInfo(@UserMessage String prompt);
  ...
  public BookInfo getBookInfo(String bookTitle) {
    // TODO [Step 4-2]: BookInfoExtractor를 AiServices.builder(...).chatModel(chatModel).build()로
    // 생성하고
    //   extractor.extractBookInfo("'" + bookTitle + "' 책에 대한 정보를 알려주세요.") 를 반환.
    BookInfoExtractor extractor =
        AiServices.builder(BookInfoExtractor.class).chatModel(chatModel).build();
    return extractor.extractBookInfo("'" + bookTitle + "' 책에 대한 정보를 알려주세요.");
  }

  @Override
  public List<BookInfo> getBooksByAuthor(String authorName) {
    // TODO [Step 4-3]: BookInfoExtractor를
    //   AiServices.builder(...).chatModel(chatModel).build()로 생성하고
    //   extractor.extractBookList("작가 '" + authorName + "'의 대표작 3권을 목록으로 알려주세요.").books() 를 반환.
    BookInfoExtractor extractor =
        AiServices.builder(BookInfoExtractor.class).chatModel(chatModel).build();
    return extractor.extractBookList("작가 '" + authorName + "'의 대표작 3권을 목록으로 알려주세요.").books();
  }
}
