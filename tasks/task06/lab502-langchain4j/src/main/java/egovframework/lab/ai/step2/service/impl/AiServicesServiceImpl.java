 ...
 *
 * <p>AiServices 패턴: 내부 인터페이스를 정의하면 LangChain4j가 런타임에 구현체를 자동 생성.
 *
 * <pre>
 * Spring AI 비교:
...
public class AiServicesServiceImpl implements AiServicesService {
  ...
  //   }
  /** 기본 챗봇: 어노테이션 없이 단순 메서드 호출 */
  private interface BasicChatbot {
    String chat(String userMessage);
  }

  /**
   * 동적 시스템 프롬프트 챗봇 @SystemMessage의 {{variable}}에 @V로 값을 바인딩. Spring AI:
  ...
  /**
   * 토픽 챗봇: @UserMessage 템플릿에 @V로 변수 바인딩 Spring AI: chatClient.prompt().user(u -> u.text("{topic}에
   * 대해...").param("topic", topic)).call().content()
   */
  private interface TopicChatbot {
    @UserMessage("{{topic}}에 대해 세 줄로 설명해주세요.")
    String chat(@V("topic") String topic);
  }
  ...
  public String chat(String message) {
    ...
    BasicChatbot bot =
        AiServices.builder(BasicChatbot.class) //
            .chatModel(chatModel)
            .build();
  ...
  public String chatWithSystemPrompt(String systemPrompt, String userMessage) {
    // TODO [Step 2-3]: AiServices.builder(DynamicSystemChatbot.class).chatModel(chatModel).build()
    // 를 사용.
    DynamicSystemChatbot bot =
        AiServices.builder(DynamicSystemChatbot.class) //
            .chatModel(chatModel)
            .build();
    return bot.chat(systemPrompt, userMessage);
  }
  ...
  public String chatAboutTopic(String topic) {
    ...
    TopicChatbot bot =
        AiServices.builder(TopicChatbot.class) //
            .chatModel(chatModel)
            .build();
    return bot.chat(topic);
  }
}
